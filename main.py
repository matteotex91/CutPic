...
def generate_gaussian_3d(ix,iy,iz,gaussian_set):
    return np.tensordot(np.tensordot(gaussian_set[ix,:],gaussian_set[iy,:],axes=0),gaussian_set[iz,:],axes=0)
...
if __name__ == "__main__":
    ...
    while not flag_max_count_achieved:
        ...
        max_count=np.sum(np.logical_and(
            np.logical_and(
            np.logical_and(
            np.logical_and(
            np.logical_and(
        ...
        laplacian=6*pad_blurred[1:-1,1:-1,1:-1]-pad_blurred[0:-2,1:-1,1:-1]-pad_blurred[2:,1:-1,1:-1]-pad_blurred[1:-1,0:-2,1:-1]-pad_blurred[1:-1,2:,1:-1]-pad_blurred[1:-1,1:-1,0:-2]-pad_blurred[1:-1,1:-1,2:]
